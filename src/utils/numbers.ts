...
const dateWithoutHours = (timestamp: number) =>
...
const getReturn = (
    ...
    return (
        parseFloat(accruedLusdValue.toString()) - parseFloat(deposit.toString())
    );
};
...
const getTokenUri = (encodedTokenUri: string): string => {
    ...
    const dataStartIndex =
        encodedTokenUri.indexOf('base64,') + 'base64,'.length;
...
const getBreakEvenPeriodInDays = (
    ...
    const effectivePremium = marketPricePremium.mul(
        Decimal.ONE.sub(claimBondFee)
    );
...
const getFutureBLusdAccrualFactor = (
    ...
    return Decimal.ONE.div(floorPrice).mul(
        duration.div(duration.add(alphaAccrualFactor))
    );
};
...
const getRebondPeriodInDays = (
    ...
    const effectivePremium =
        Decimal.ONE.sub(claimBondFee).mul(marketPricePremium);
    if (effectivePremium.lte(Decimal.ONE)) return Decimal.INFINITY;
    const sqrt = Decimal.from(
        Math.sqrt(parseFloat(effectivePremium.toString()))
    );
...
const getAverageBondAgeInSeconds = (
    ...
    const averageStartTimeMs =
        Math.round(
            parseFloat(
                totalWeightedStartTimes.div(pendingBucketLusd).toString()
            )
        ) * 1000;
    const averageBondAgeInSeconds = Decimal.from(
        Date.now() - averageStartTimeMs
    ).div(1000);
...
const getRemainingRebondOrBreakEvenDays = (
    ...
    const daysUntilControllerStartsAdjusting = toFloat(
        getDaysUntilControllerStartsAdjusting(
            averageBondAgeInSeconds,
            targetBondAgeInSeconds
        )
    );
    const rebondOrBreakEvenDaysRemaining =
        toFloat(rebondOrBreakEvenPeriodInDays) - bondAgeInDays;

    if (rebondOrBreakEvenDaysRemaining < daysUntilControllerStartsAdjusting) {
        return rebondOrBreakEvenDaysRemaining;
    }

    const lambertDividend =
        Math.log(0.99) * toFloat(rebondOrBreakEvenPeriodInDays);
    const lambertDivisor =
        0.99 ** (bondAgeInDays + daysUntilControllerStartsAdjusting);
    const lambertQuotient = lambertW0(-(lambertDividend / lambertDivisor));

    const formulaDividend =
        lambertQuotient +
        Math.log(0.99) * (bondAgeInDays + daysUntilControllerStartsAdjusting);

    const formulaDivisor = Math.log(0.99);

    const daysUntilBondReachesRebondOrBreakEven = -(
        formulaDividend / formulaDivisor
    );
