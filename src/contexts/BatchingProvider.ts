...
interface BatchedCalls {
    ...
    calls: CallRequest[];
    callbacks: [
        (resolveValue: string) => void,
        (rejectReason: unknown) => void
    ][];
}

const emptyBatch = (): BatchedCalls => ({
    blockTag: 'latest',
    calls: [],
    callbacks: [],
});
...
export class BatchedProvider extends BaseProvider {
    ...
    constructor(
        underlyingProvider: BaseProvider,
        chainId = 0,
        batchingDelayMs = 10
    ) {
    ...
    _alreadyBatchedCallsConflictWith(blockTag: BlockTag) {
        return (
            ...
            blockTag !== this._batched.blockTag
        );
    }
    ...
    private async _performGetBalance(
        ...
        return this._multicall.getEthBalance(params.address, {
