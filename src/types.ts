...
type BondEventTransitions = Record<
    BondView,
    Partial<Record<BondEvent, BondView>>
>;
...
export type OptimisticBond = Pick<
    Bond,
    'id' | 'deposit' | 'startTime' | 'status'
>;
...
export type BondTransactionStatuses = Record<
    BondTransaction,
    TransactionStatus
>;
...
export type BLusdLpRewards = Array<{
    name: string;
    address: string;
    amount: Decimal;
}>;
